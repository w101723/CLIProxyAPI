...
func (w *Watcher) reloadClients(rescanAuth bool, affectedOAuthProviders []string, forceAuthRefresh bool) {
	...
	geminiAPIKeyCount, vertexCompatAPIKeyCount, claudeAPIKeyCount, codexAPIKeyCount, qwenRerankAPIKeyCount, openAICompatCount := BuildAPIKeyClients(cfg)
	totalAPIKeyClients := geminiAPIKeyCount + vertexCompatAPIKeyCount + claudeAPIKeyCount + codexAPIKeyCount + qwenRerankAPIKeyCount + openAICompatCount
	...
	totalNewClients := authFileCount + geminiAPIKeyCount + vertexCompatAPIKeyCount + claudeAPIKeyCount + codexAPIKeyCount + qwenRerankAPIKeyCount + openAICompatCount
	...
	log.Infof("full client load complete - %d clients (%d auth files + %d Gemini API keys + %d Vertex API keys + %d Claude API keys + %d Codex keys + %d Qwen rerank keys + %d OpenAI-compat)",
		totalNewClients,
		authFileCount,
		geminiAPIKeyCount,
...
func BuildAPIKeyClients(cfg *config.Config) (int, int, int, int, int, int) {
	...
	if len(cfg.QwenRerankKey) > 0 {
		qwenRerankAPIKeyCount += len(cfg.QwenRerankKey)
	}
	...
	return geminiAPIKeyCount, vertexCompatAPIKeyCount, claudeAPIKeyCount, codexAPIKeyCount, qwenRerankAPIKeyCount, openAICompatCount
}
